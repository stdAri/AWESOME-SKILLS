...
本项目是 Claude AI Research Skill 的汇总追踪仓库。目的是集中收集、对比和管理各类与 Claude Code 学术研究能力相关的开源项目和技能（skills）。
...
├── Auto-claude-code-research-in-sleep/  # 自动化研究技能（OpenAlex、Semantic Scholar、Gemini、Exa 等多源检索）
├── autoresearch/               # AI 自动化科研项目（Python ML 训练/分析）
├── claude-scholar/             # Claude Scholar 插件（commands、hooks、skills、MCP 集成）
├── sciwrite/                   # 科学写作技能
├── skills/                     # 通用技能合集
